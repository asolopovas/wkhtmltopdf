...
Linux `.deb` packages recommend ImageMagick to decode AVIF images for the bundled Qt WebKit renderer, but it is optional so `dpkg -i` can install the package without fetching additional dependencies.
